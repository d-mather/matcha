...
try {
    ...
    while ($result1 = $sql->fetch(PDO::FETCH_ASSOC)) {
        if ($result1['username'] == $login || $result1['meta'] != 2) {
            ...
            $tmp_array_pic = null;
            $sql2 = $conn->prepare('SELECT username, likes, views, who_liked, blocked, who_blocked, visited, connected FROM `public`');
            $sql2->execute();
            while ($result4 = $sql2->fetch(PDO::FETCH_ASSOC)) {
                if ($login == $result4['username']) {
                    ...
                    if (!$result4['connected']) {
                        $result4['connected'] = '';
                    }
                    $tmp_array = array('likes' => $result4['likes'], 'views' => $result4['views'], 'who_liked' => $result4['who_liked'], 'blocked' => $result4['blocked'], 'who_blocked' => $result4['who_blocked'], 'visited' => $result4['visited'], 'connected' => $result4['connected']);
                    $user_tmp_array = array_merge($user_tmp_array, $tmp_array);
                    break;
                }
            }
